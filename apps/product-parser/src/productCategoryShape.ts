...
export const productCategoryShape = z.object({
  ...
  product_type: z.string().max(750, 'Product type must have at most 750 characters.'),
});
...
export const productCategoryTransform = <T extends z.infer<typeof productCategoryShape>>({
  ...
  if (google_product_category) {
    const categoryById = categoriesIdentifiers.find(
      (identifier) => identifier === google_product_category,
    );
